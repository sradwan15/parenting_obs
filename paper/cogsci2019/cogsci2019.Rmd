...
kable(e1, digits=2, caption="Lexical diversity measures in Experiment 1.", label="e1tab")
```
...
There was suggestively lower TTR in the Video condition (mean: 0.38) than in the Control condition (mean: 0.47, $\beta=`r e2TTR["conditionexp","beta"]`$, t(8.6) = `r e2TTR["conditionexp","zval"]`, *p*=.009). 
There was no significant effect of age, nor a significant interaction.
The preregistered regression predicting the number of tokens used by parents found no significant effects. 
...
kable(e2, digits=2, caption="Lexical diversity measures in Experiment 2.", label="e2tab")
```
...
For the preregistered analyses, we fit mixed-effects linear regressions predicting the number of parental bids for joint attention and the total number of episodes of JA. 
These JA variables were predicted as a function of fixed effects of condition, age (0-centered), and their interaction, with random intercepts per video.
Parents made significantly more bids for JA in the Video condition (mean: `r round(subset(e2_bids, Condition=="Video")$mean,2)`, bootstrapped 95% CI: `r round(subset(e2_bids, Condition=="Video")$ci_lower,2)`, `r round(subset(e2_bids, Condition=="Video")$ci_upper,2)`) than in the Control condition (mean: `r round(subset(e2_bids, Condition=="Control")$mean,2)`, 95% CI: `r round(subset(e2_bids, Condition=="Control")$ci_lower,2)`, `r round(subset(e2_bids, Condition=="Control")$ci_upper,2)`, $\beta=`r e2bids["ConditionVideo","beta"]`$, t(79) = `r e2bids["ConditionVideo","zval"]`, *p*<.001). 
There were no other significant effects on parental bids for JA.

There were significant effects of condition, age, and 

Figure \ref{fig:e2ja} shows the mean number of parental bids for JA by condition (left), along with total episodes of JA by age and condition (right).

Additional exploratory regressions with a similar structure were carried out on number and duration of coordinated and passive JA episodes.

```{r fig:e2ja, fig.env = "figure", fig.pos = "H", fig.align='center', fig.width=6, fig.height=3.0, set.cap.width=T, num.cols.cap=1, fig.cap = "\\label{fig:e2ja} Mean number of bids for JA by condition (left) and total episodes of JA by age and condition (right) in Experiment 2."}
...
There was an effect on total bids for joint attention. 
