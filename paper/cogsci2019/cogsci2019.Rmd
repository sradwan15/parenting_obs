...
Children want and need to play a lot: 
by interacting with other people or physcial objects, they explore, experiment and build up their understandings of how the world functions. 
Researches have suggested that play is important to learning,
"allowing children to imitate adult behaviors, practice motor skills, process emotional events, and learn much about their world."(Hirsh,2008)
## Guided Play Scaffolds Learning
- guided play activities can scaffold children's language and reasoning
What we cannot ignore, however,
is that children's efforts to explore the world are often "assisted and fostered by others who are more skillful than they are"(Kaye, 1970). 
With the help of an adult or "expert", children are able to engage with more advanced play and explore the functions of objects more.
These tutorial interactions have been shown to be a "a crucial feature of infancy and childhood"(Wood, 1970).
Thus, with the knowledge of both play and tutorial interactions, guided play, "where children actively engage in pleasurable and seemingly spontaneous activities under the direction of adults"(Hirsh,2008), 
have drawn researchers' interest. 
Weisberg showed that, with preschoolders, guided play scaffolds the environment while still allowing children to maintain a large degree of control, 
and it outperforms direct-instruction approaches in encouraging a variety of positive academic outcomes (Weisberg,Hirsh&Golinkoff, 2013).
...
increased during the home-visiting but did not sustain after four months of intervention [there are more detailed sustainibility differences of these measure. should I make it more clear?] (Suskind, 2015).
...
### Participants. 
60 infants aged 6-24 months and their parents participated in a museum in northern California. 
...
Participants were randomly assigned to an Activity Video condition or No-Video Control condition. 
...
The data was analyzed according to our preregistration. [link]
...
According to Jarvis2010, MTLD is a better measure of lexical diversity. 
...
```{r e2TTR, fig.env = "figure", fig.pos = "H", fig.align='center', fig.width=2, fig.height=2, set.cap.width=T, num.cols.cap=1, fig.cap = "How to do subfigures?"}
img <- png::readPNG("figs/e2_TTR.png")
grid::grid.raster(img)
```

```{r e2MTLD, fig.env = "figure", fig.pos = "H", fig.align='center', fig.width=2, fig.height=2, set.cap.width=T, num.cols.cap=1, fig.cap = "One column image."}
img <- png::readPNG("figs/e2_MTLD.png")
grid::grid.raster(img)
```
