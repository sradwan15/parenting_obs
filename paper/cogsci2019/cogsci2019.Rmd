...
get_lmer_reporting_values <- function(lmer.fit, digits=2) {
  ...
  # not sure how to pull estimated degrees of freedom...
  return(cbind(beta, se, zval, pval))
}
require("lmerTest")
...
summary(bmodMTLD) # condition mean=-8.7 sd=3.3 2.5%=-15.1 97.5%=-20
...
There was significantly lower TTR in the Video condition (mean: `r round(subset(e1_lexdiv, condition=='exp')$TTR, 2)`) than in the Control condition (mean: `r round(subset(e1_lexdiv, condition=='con')$TTR, 2)`, $\beta=$`r e1TTR["conditionexp","beta"]`, t(52.4) = `r e1TTR["conditionexp","zval"]`, *p*<.001). 
There were no significant effects of age, gender, or parent's level of education.
A similar mixed-effects linear regression instead predicting MTLD also found significantly lower lexical diversity in the Video condition (mean MTLD: `r round(subset(e1_lexdiv, condition=='exp')$MTLD, 2)`) than in the Control condition (mean: `r round(subset(e1_lexdiv, condition=='exp')$TTR, 2)`, $\beta=$`r e1MTLD["conditionexp","beta"]`, t(55) = `r e1MTLD["conditionexp","zval"]`, *p*=.001), with no other significant effects.
...
```{r fig:e1lex_div, fig.env = "figure", fig.pos = "H", fig.align='center', fig.width=3.5, fig.height=3.0, set.cap.width=T, num.cols.cap=1, fig.cap = "\\label{fig:e1lex_div} Mean lexical diversity scores by condition (left: Type/Token ratio, right: MTLD) in Experiment 1. Error bars show bootstrapped 95 percent confidence intervals (CIs)."}
...
e1ttr <- ggplot(ms_lex, aes(x=Condition, y=mean, fill=Condition)) + geom_bar(stat="identity") + 
  ...
  geom_point(data=d, aes(x=Condition, y=TTR), alpha=.3, position = position_jitterdodge()) +
...
e1mtld <- ggplot(ms_lex_mtld, aes(x = Condition, y = mean, fill = Condition)) + geom_bar(stat="identity") + 
  ...
  geom_point(data=d, aes(x=Condition, y=MTLD), alpha=.3, position = position_jitterdodge()) +
...
e1tokens <- ggplot(ms_tok, aes(x=Condition, y = mean, fill = Condition)) + geom_bar(stat="identity") + 
  geom_linerange(aes(ymin = ci_lower, ymax = ci_upper), position = position_dodge(width = .9)) +
  geom_point(data=d, aes(x=Condition, y=tokens), alpha=.3, position = position_jitterdodge()) +
...
e1types <- ggplot(ms_type, aes(x = Condition, y = mean, fill = Condition)) + geom_bar(stat="identity") + 
  geom_linerange(aes(ymin = ci_lower, ymax = ci_upper), position = position_dodge(width = .9)) +
  geom_point(data=d, aes(x=Condition, y=types), alpha=.3, position = position_jitterdodge()) +
...
There were significantly more bids for JA in the Video condition (mean: `r round(subset(e1bids_M, Condition=="Video")$mean,2)`, sd: `r round(subset(e1bids_M, Condition=="Video")$sd,2)`) than in the Control condition (mean: `r round(subset(e1bids_M, Condition=="Control")$mean,2)`, sd: `r round(subset(e1bids_M, Condition=="Control")$sd,2)`, $\beta=`r e1bids["ConditionVideo","beta"]`$, t(40.3) = `r e1bids["ConditionVideo","zval"]`, *p*<.01). 
...
```{r fig:e1ja, fig.env = "figure", fig.pos = "H", fig.align='center', fig.width=3.5, fig.height=3.0, set.cap.width=T, num.cols.cap=1, fig.cap = "\\label{fig:e1ja} Mean number of bids (left) and episodes (right) of joint attention by condition in Experiment 1."}
...
e1bids <- ggplot(ms_bids, aes(x = Condition, y = mean, fill = Condition)) + geom_bar(stat="identity") + 
  geom_linerange(aes(ymin = ci_lower, ymax = ci_upper), position = position_dodge(width = .9)) +
  geom_point(data=d, aes(x=Condition, y=bids_tot), alpha=.3, position = position_jitterdodge()) +
...
e1tja <- ggplot(ms_tja, aes(x = Condition, y = mean, fill = Condition)) + geom_bar(stat="identity") + 
  geom_linerange(aes(ymin = ci_lower, ymax = ci_upper), position = position_dodge(width = .9)) +
  geom_point(data=d, aes(x=Condition, y=total_ja), alpha=.3, position = position_jitterdodge()) +
...
e1pja <- ggplot(ms_pja, aes(x = Condition, y = mean, fill = Condition)) + geom_bar(stat="identity") + 
  geom_linerange(aes(ymin = ci_lower, ymax = ci_upper), position = position_dodge(width = .9)) +
  geom_point(data=d, aes(x=Condition, y=pja), alpha=.3, position = position_jitterdodge()) +
...
e1cja <- ggplot(ms_cja, aes(x = Condition, y = mean, fill = Condition)) + geom_bar(stat="identity") + 
  geom_linerange(aes(ymin = ci_lower, ymax = ci_upper), position = position_dodge(width = .9)) +
  geom_point(data=d, aes(x=Condition, y=cja), alpha=.3, position = position_jitterdodge()) +
...
There was a main effect of condition on total bids for joint attention (JA). 
Parents in the Video condition, after seeing a video demonstrating an activity, made a greater number of bids for JA with their child.
There was no effect of condition on the number of episodes of either passive or coordinated JA, or the duration of these episodes. 
There was a marginal effect of gender on bids for JA, with parents of males producing more bids. 
There was a marginal interaction between RR scores and condition on passive JA, such that the experimental condition increased the number of episodes of passive JA to a greater extent for people with high RR scores.
...
### Participants. 
`r nrow(d)` infants (F = `r e2_gender['F']`, M = `r e2_gender['M']`) aged 12-24 months (42 12-17.9 month-olds, 42 18-24 month-olds) and their parents participated in the same museum as Experiment 1. 
...
Parents' child-directed speech was transcribed and processed according to the same procedure used in Experiment 1, and the preregistered regressions predicting TTR and number of tokens were carried out.
...
We fit a mixed-effects linear regression predicting TTR as a function of age (scaled and 0-centered) and condition with an interaction term, and with random intercepts per video using lme4 [@lme4].
There was significantly lower TTR in the Video condition (mean: 0.38) than in the Control condition (mean: 0.47, $\beta=`r e2TTR["conditionexp","beta"]`$, t(8.7) = `r e2TTR["conditionexp","zval"]`, *p*=.01).
There was no significant effect of age, nor a significant interaction.
The preregistered regression of 
An exploratory similar mixed-effects linear regression instead predicting MTLD found no significant effects of age or condition.
...
kable(e2, digits=2, caption="Lexical diversity measures in Experiment 2.")
```
...
d$Condition <- factor(d$condition, levels = c("con","exp"), labels = c("Control","Video"))
e2ja = d
e2ja$age = as.numeric(langcog::scale(e2ja$age, scale=FALSE)) # 0-centered but NOT scaled -- change text


# Total number of bids
bids_mod <- lmer(bids ~ age * Condition + (1 | video), data = e2ja)
summary(bids_mod) # singular fit, convergence code: 0
e2bids <- get_lmer_reporting_values(bids_mod)
# Condition est = 3.25 t(79) = 3.65 p<.001
bids_bmod <- stan_glmer(bids ~ age * Condition + (1|video), data=e2ja)
summary(bids_bmod) # significant effect of condition! mean=3.2, 95%CI = [1.2, 5.2]

tja_mod <- lmer(total_ja ~ age * Condition + (1 | video), data = e2ja)
summary(tja_mod) # singular fit, convergence code: 0
#                    Estimate Std. Error      df t value  Pr(>|t|) 
# age                 -4.6051     1.3927 79.0000  -3.306  0.00142 ** 
# ConditionVideo       1.5253     0.5626 79.0000   2.711  0.00822 ** 
# age:ConditionVideo   5.5193     2.1893 79.0000   2.521  0.01371 *  

tja_bmod <- stan_glmer(total_ja ~ age * Condition + (1|video), data=e2ja)
summary(tja_bmod) # age n.s., sig effect of condition: mean=3.2 95%CI = [1.1, 5.2]
#                                        mean    sd     2.5%   25%    50%    75%    97.5%
# (Intercept)                             7.3    0.5    6.3    6.9    7.2    7.6    8.2 
# age                                    -4.3    1.5   -7.2   -5.3   -4.3   -3.3   -1.3 
# ConditionVideo                          1.5    0.7    0.2    1.1    1.5    1.9    2.9
...
summary(cja_mod) #  Condition: t(79) = 2.47 beta=1.33 p=.02; age*Condition beta=4.92 t(79) = 2.35 p=.02
# also singular fit, convergence code: 0 - check Bayesian regression
...
summary(pja_time_mod) # singular - convergence code: 0, age beta=-15512 t(79) = 2.40 p<.05
pjat_bmod <- stan_glmer(pja_length ~ age * Condition + (1|video), data=e2ja)
summary(pjat_bmod)
#                                        mean       sd         2.5%       25%        50%        75%        97.5%   
#(Intercept)                             11247.4     2193.1     6977.7     9822.1    11239.1    12655.2    15736.7
#age                                    -13924.2     7097.9   -27467.6   -18661.5   -14163.6    -9341.5      722.4
#ConditionVideo                            824.8     3098.5    -5399.6    -1190.0      846.8     2877.5     6867.3
#age:ConditionVideo                        108.3    11738.0   -23881.5    -7171.3      411.3     7778.3    22058.5

# Total duration of coordinated joint attention.
cja_time_mod <- lmer(cja_length ~ age * condition + (1 | video), data = e2ja)
summary(cja_time_mod) # singular - convergence code: 0, marginal age: beta=31757 t(79) = 1.81 p=.07
cjat_bmod <- stan_glmer(cja_length ~ age * Condition + (1|video), data=e2ja)
summary(cjat_bmod)
#                                           mean        sd          2.5%        25%         50%         75%      
#(Intercept)                              73053.4      5611.1     62252.3     69338.3     73049.6     76773.7
#age                                      28879.1     19079.4     -8533.5     16385.2     29112.3     41487.9
#ConditionVideo                            5745.4      7874.5     -9994.6       535.3      5725.3     11163.4
#age:ConditionVideo                       45792.3     30918.9    -14154.8     25281.7     45101.1     66074.2
...
As specified in our preregistration, we fit five mixed-effects linear regressions predicting the following JA variables: parental bids, the number of episodes of coordinated JA and passive JA, and the total duration of coordinated and passive JA.
These JA variables were predicted as a function of fixed effects of condition, age (0-centered), with random intercepts per video.
There were significantly more bids for JA in the Video condition (mean: `r round(subset(e2_bids, Condition=="Video")$mean,2)`, bootstrapped 95% CI: `r round(subset(e2_bids, Condition=="Video")$ci_lower,2)`, `r round(subset(e2_bids, Condition=="Video")$ci_upper,2)`) than in the Control condition (mean: `r round(subset(e2_bids, Condition=="Control")$mean,2)`, 95% CI: `r round(subset(e2_bids, Condition=="Control")$ci_lower,2)`, `r round(subset(e2_bids, Condition=="Control")$ci_upper,2)`, $\beta=`r e2bids["ConditionVideo","beta"]`$, t(79) = `r e2bids["ConditionVideo","zval"]`, *p*<.001). 
There were no other significant effects.
There were no significant effects on the number or total duration of coordinated JA episodes, nor on the total duration of passive JA episodes. 

Figure \ref{fig:e2ja} shows the mean number of bids and episodes of JA by condition.

```{r fig:e2ja, fig.env = "figure", fig.pos = "H", fig.align='center', fig.width=3.5, fig.height=3.0, set.cap.width=T, num.cols.cap=1, fig.cap = "\\label{fig:e2ja} Mean number of bids and episodes of JA by condition in Experiment 2."}


e2bids <- ggplot(e2_bids, aes(x = Condition, y = mean, fill = Condition)) + geom_bar(stat="identity") + 
  geom_linerange(aes(ymin = ci_lower, ymax = ci_upper), position = position_dodge(width = .9)) +
  geom_point(data=d, aes(x=Condition, y=bids), alpha=.3, position = position_jitterdodge()) +
  xlab("Condition") + ylab("Total Bids for Joint Attention") +
  langcog::scale_colour_solarized() + ggthemes::theme_few() + 
  theme(legend.position="none") 
...
```{r e2ja-pass-coord, fig.env = "figure", fig.pos = "H", fig.align='center', fig.width=3.5, fig.height=3.0, set.cap.width=T, num.cols.cap=1, fig.cap = "\\label{fig:e2ja-pass-coord} Mean number of passive (left) and coordinated (right) episodes of JA by condition in Experiment 2."}
ms_pja <- d %>% group_by(Condition) %>%
  tidyboot_mean(pja) 
...
There was a marginal effect of condition on total bids for joint attention. 
Parents in the experimental condition (i.e., those who saw a video demonstrating an activity) made a greater number of bids for joint attention with their child.
