...
### Participants. 
`r nrow(e1ld)` infants (F = `r e1_gender['F']`, M = `r e1_gender['M']`) aged 6-24 months (20 6-11.9 month-olds, 20 12-17.9 month-olds, and 20 18-24 month-olds) and their parents participated in a museum in northern California. 
...
e1ttr <- ggplot(e1_lex, aes(x=Condition, y=mean, fill=Condition)) + geom_bar(stat="identity") + 
  ...
  theme(legend.position="none") + ggtitle("Experiment 1")
...
e2ttr <- ggplot(ttr2, aes(x=Condition, y=mean, fill=Condition)) + geom_bar(stat="identity") + # labels=addline_format(c("Science Video", "Activity Video"))
  ...
  xlab("Condition") + ylab("Type/Token Ratio (TTR)") +
  langcog::scale_fill_solarized() + ggthemes::theme_few() +
  theme(legend.position="none") + ggtitle("Experiment 2")
...
e1bids_mod <- stan_glmer(paste("bids_tot", model_string), data = e1ja) # 
summary(e1bids_mod) # condition beta = 2.6, t(40.3) = 2.95 p<.01, gender p=.05 ?
...
e1tja_mod <- stan_lmer(paste("total_ja", model_string), data = e1ja)
summary(e1tja_mod) # NA
e1tjadur_mod <- stan_lmer(paste("total_lja", model_string), control = list(adapt_delta = 0.99), data = e1ja)
summary(e1tjadur_mod) # 
...
e1cja_mod <- stan_lmer(paste("cja", model_string), control = list(adapt_delta = 0.99), data = e1ja) 
...
Looking at the most frequent content words parents used after the Activity Video, ...
In contrast, the most frequent content words parents used in the Control condition were...
...
ttr_means <- e2data %>% group_by(Condition) %>%
  summarise(mean = mean(TTR), sd = sd(TTR))
cohens_d(ttr_means)
```
...
However, a significant interaction of age and condition ($\beta=`r e2tjaAgeCond["age:ConditionActivity Video"]`$, 95% CI = [`r e2tjaAgeCond["95pLB"]`,`r e2tjaAgeCond["95pUB"]`]), shown in Figure \ref{fig:JA} (right bottom), demonstrates that older children in the Activity Video condition did not see a decrease in the number of episodes of JA.
