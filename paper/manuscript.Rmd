...
d = read.csv("exp1_lexical_diversity_data.csv")
...
d = read.csv("exp2_lexical_diversity_data.csv")
...
p_direction(modTTRE, effects="all") # EL 84.9%, RR 83.3%, parent_ed 91.7%

modMTLDE <- stan_glmer(paste("MTLD", e1model_string), data=e1ld)
summary(modMTLDE) #
p_direction(modMTLDE, effects="all") # EL 78.5% AA 89.2% RR 86.6% genderM 87.8% parent_ed 91.5%

tokens_modE <- stan_glmer(paste("tokens", e1model_string),  data=e1ld) 
summary(tokens_modE) # condition 54.4 
get_stan_glmer_reporting_values(tokens_modE, 'parent_ed') # SHOULD REPORT
p_direction(tokens_modE, effects="all") # RR 75.7% parent_ed 95.1%

# predict word types
types_modE <- stan_glmer(paste("types", e1model_string), data=e1ld) 
summary(types_modE) 
p_direction(types_modE, effects="all") # EL 87.2% AA 77.7% genderM 88.3%, parent_ed 91.7% cond:AA 94.5%
get_stan_glmer_reporting_values(types_modE, 'parent_ed') 
get_stan_glmer_reporting_values(types_modE, 'conditionexp:AA') 
```
...
For effects that are at least 95% likely to be non-zero according to the posterior distribution, we report estimated coefficients ($\beta$) as well as 95% Bayesian credible intervals (95% CI), demarcating the range within which 95% of the posterior falls.
...
summary(e1bids_modE) # cond, almost gender 
...
get_stan_glmer_reporting_values(e1pja_modE, "RR") # -1.09, 95% CI = [-2.22,0.07] % report % posterior <0?
...
There were no notable effects on the number of or total duration of coordinated or passive JA episodes.
...
Words that are strongly indicative of being from the Activity Video condition include "give", "big", "small", "ribbit", "thank", "have", and "bus", while words that are most characteristic of the No Video condition include "shake", "ready", "oh", "on", "going", "going", "see", "let", and the child's name. 
...
# predict word tokens
e2tokens_m <- stan_lmer(log(tokens) ~ age * condition + (1 | video), data=e2data)
summary(e2tokens_m) # NA 
print(e2tokens_m, digits=2)
p_direction(e2tokens_m, effects="all") 
# conditionexp 90.3%, age:conditionexp 83.1%  video:big_small 96.3%, video:orchestra 94.5%
...
This revealed lower TTR after the Activity Video (mean: `r round(e2_lexdiv[2,]$TTR, 2)`) than after the Science Video (mean: `r round(e2_lexdiv[1,]$TTR, 2)`, $\beta=`r e2TTR["conditionexp"]`$, , 95% CI=[`r e2TTR['95pLB']`,`r e2TTR['95pUB']`]). 
...
Shown in Figure \ref{fig:JA} (left bottom), parents made notably more bids for JA after watching the Activity Video (mean: `r round(subset(e2_bids, Condition=="Activity Video")$mean,2)`, 95% conf. int.: [`r round(subset(e2_bids, Condition=="Activity Video")$ci_lower,2)`, `r round(subset(e2_bids, Condition=="Activity Video")$ci_upper,2)`]; $\beta=`r e2bids["ConditionActivity Video"]`$, 95% CI = [`r e2bids["95pLB"]`,`r e2bids["95pUB"]`]) than after the Science Video (mean: `r round(subset(e2_bids, Condition=="Science Video")$mean,2)`, 95% conf. int.: [`r round(subset(e2_bids, Condition=="Science Video")$ci_lower,2)`, `r round(subset(e2_bids, Condition=="Science Video")$ci_upper,2)`]). 
There were no other effects on parental bids for JA.

The regression predicting the number of episodes of JA revealed an effect of condition ($\beta=`r e2tjaCond["ConditionActivity Video"]`$, 95% CI = [`r e2tjaCond["95pLB"]`,`r e2tjaCond["95pUB"]`]), with more episodes of JA occurring after the Activity Video (mean: `r round(subset(e2_tja, Condition=="Activity Video")$mean,2)`, 95% conf. int.: [`r round(subset(e2_tja, Condition=="Activity Video")$ci_lower,2)`, `r round(subset(e2_tja, Condition=="Activity Video")$ci_upper,2)`]) than after the Control Video (mean: `r round(subset(e2_tja, Condition=="Science Video")$mean,2)`, 95% conf. int.: [`r round(subset(e2_tja, Condition=="Science Video")$ci_lower,2)`, `r round(subset(e2_tja, Condition=="Science Video")$ci_upper,2)`]).
Older children also participated in fewer episodes of JA ($\beta=`r e2tjaAge["age"]`$, 95% CI = [`r e2tjaAge["95pLB"]`,`r e2tjaAge["95pUB"]`]). 
However, this age effect was moderated in the Activity Video condition ($\beta=`r e2tjaAgeCond["age:ConditionActivity Video"]`$, 95% CI = [`r e2tjaAgeCond["95pLB"]`,`r e2tjaAgeCond["95pUB"]`]): shown in Figure \ref{fig:JA} (right bottom), older children did not engage in fewer episodes of JA after an activity video.
...
Older children both engaged in more episodes of passive JA with their caregiver ($\beta=`r e2pja["age"]`$, 95% CI = [`r e2pja["95pLB"]`,`r e2pja["95pUB"]`]), and spent more total time in passive JA with their caregiver ($\beta=`r e2pjat["age"]`$, 95% CI = [`r e2pjat["95pLB"]`,`r e2pjat["95pUB"]`]).
...
The words that are strongly indicative of being from the Activity Video condition include "big", "little", "give", "small", "cow", "yellow", "take", and "put", while words that are most characteristic of the Science Video condition include "beep", "like", "neigh", "uhoh", "say", "for", "does", and "did". 
