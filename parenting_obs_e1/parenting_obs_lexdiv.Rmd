...
ms_lex <- d %>%
  group_by(condition) %>%
  multi_boot_standard(col = "lexdiv") # Type/Token Ratio
...
ggplot(ms_lex, aes(x = condition, y = mean, fill = condition)) + 
  ...
  ggthemes::theme_few() 
ggsave(file="e1_TTR.pdf", width=3, height=3)
```
...
ggplot(ms_tok, aes(x = condition, y = mean, fill = condition)) + 
  ...
  langcog::scale_colour_solarized()  +
  ggthemes::theme_few() 
ggsave(file="e1_word_tokens.pdf", width=3, height=3)
```
...
ggplot(ms_tok, aes(x = condition, y = mean, fill = condition)) + 
  ...
  langcog::scale_colour_solarized()  +
  ggthemes::theme_few()
ggsave(file="e1_word_types.pdf", width=3, height=3)
```
